...
```json
{
  ...
  "Port": 7777,
  "Password": "",
  "MasterServerUrl": "http://203.0.113.20:5000",
  "MasterToken": "ein-langes-geheimes-token",
  "Map": "Default",
  "SaveGame": "New session"
}
```

`BindAddress` ist die lokale Netzwerkadresse, auf der der Server lauscht. `0.0.0.0` bedeutet alle Interfaces. `PublicAddress` ist die von den Spielern erreichbare IP oder DNS-Adresse und darf nicht `0.0.0.0` sein. TCP `Port` muss in der Firewall freigegeben sein.
...
```json
{
  "BindAddress": "0.0.0.0",
  "Port": 5000,
  "AuthToken": "ein-langes-geheimes-token"
}
```

`AuthToken` muss in `master.json` und `server.json` identisch sein. Der Dedicated Server sendet ihn im Header `X-Master-Token`; ohne gültiges Token lehnt der Master die Registrierung mit HTTP `401` ab. Die Serverliste selbst bleibt öffentlich lesbar, damit Clients sie abrufen können.
